...
# LICENSE:
#
# Copyright (c) 2020-2023 by Ixtalo, ixtalo@gmail.com
#
...
class Archiver:
    ...
    def __init__(self):
        """Archives handling."""
        self.rm_command = "del /q" if _is_ms_windows() else "rm"
...
class ArchiverRar(Archiver):
    ...
    def build_rm_command(self, filepath: Path) -> str:
        ...
        # "xyz.rar" -> ["xyz.rar", "xyz.r"*]
        if not ArchiverRar.is_volume_part(filepath.name):
            if Path(filepath.with_suffix(".r00")).exists():
                return f'{self.rm_command} "{filepath.resolve()}" "{filepath.with_suffix("").resolve()}".r*'
            # else
        ...
        return f'{self.rm_command} "{filepath.with_name(f"{basename}").resolve()}".part*.rar'
...
class Archiver7z(Archiver):
    ...
    def build_rm_command(self, filepath: Path) -> str:
        ...
        return f'{self.rm_command} "{filepath.with_name(f"{basename}").resolve()}".7z*'
